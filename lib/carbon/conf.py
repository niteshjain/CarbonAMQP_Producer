...
defaults = dict(
  ...
  MAX_QUEUE_SIZE=1000,
  ENABLE_AMQP_CON=False,
  AMQP_CON_VERBOSE=False,
  BIND_PATTERNS=['#'],
  AMQP_CON_QUEUE='graphite_queue',
  AMQP_PUB_QUEUE='graphite_queue', 
...
class CarbonAMQPPublisherOptions(CarbonCacheOptions):
...
class CarbonAggregatorOptions(CarbonAMQPPublisherOptions):

    optParameters = [
        ["rules", "", None, "Use the given aggregation rules file."],
        ["rewrite-rules", "", None, "Use the given rewrite rules file."],
        ] + CarbonAMQPPublisherOptions.optParameters

    def postOptions(self):
        CarbonAMQPPublisherOptions.postOptions(self)
...
class CarbonRelayOptions(CarbonAMQPPublisherOptions):
