...
def createBaseService(config):
    ...
    use_amqp_con = settings.get("ENABLE_AMQP_CON", False)
    if use_amqp_con:
        from carbon import amqp_listener

        amqp_host = settings.get("AMQP_CON_HOST", "localhost")
        amqp_port = settings.get("AMQP_CON_PORT", 5672)
        amqp_user = settings.get("AMQP_CON_USER", "guest")
        amqp_password = settings.get("AMQP_CON_PASSWORD", "guest")
        amqp_verbose  = settings.get("AMQP_CON_VERBOSE", False)
        amqp_vhost    = settings.get("AMQP_CON_VHOST", "/")
        amqp_spec     = settings.get("AMQP_CON_SPEC", None)
        amqp_exchange_name = settings.get("AMQP_CON_EXCHANGE", "graphite")
...
def createPublisherService(config):
    from carbon.cache import MetricCache
    from carbon.conf import settings
    ...
    #Create Publisher service here
    from carbon.amqp_publisher import PublisherService    
