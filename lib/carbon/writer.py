...
def writeCachedDataPoints():
  ...
  while MetricCache:
    ...
    for (metric, datapoints) in optimalWriteOrder():
      dataWritten = True
      metricList.append((metric, datapoints))
      #pointCount = len(datapoints)
      #instrumentation.increment('committedPoints', pointCount)   
    if dataWritten:
      #try:
      writeMetricBaby(metricList, settings.AMQP_PUB_HOST, settings.AMQP_PUB_PORT, settings.AMQP_PUB_USER, settings.AMQP_PUB_PASSWORD, settings.AMQP_PUB_VHOST, settings.AMQP_EXCHANGE)
      #log.msg("Back in writer.py")  
      #t2 = time.time()
      #updateTime = t2 - t1    
      #instrumentation.append('updateTimes', updateTime)  
      #if settings.LOG_UPDATES:
        #log.updates("Published %d metrics in %.5f seconds" % (len(metricList), updateTime))
      #except:
        ...
        #log.msg("Failed to publish to RabbitMQ. Pushing the metrics back to cache")
    else:
      time.sleep(0.5)
...
#        writeMetricBitch(metric, datapoints, settings.AMQP_PUB_HOST, settings.AMQP_PUB_PORT, settings.AMQP_PUB_USER, settings.AMQP_PUB_PASSWORD, settings.AMQP_PUB_VHOST, settings.AMQP_EXCHANGE)
#        t2 = time.time()
#        updateTime = t2 - t1
#      except:
#        log.msg("Error writing %s" % (metric))
#        log.err()
